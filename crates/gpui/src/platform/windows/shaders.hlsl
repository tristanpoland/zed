...
struct ExternalTexture {
    ...
    Bounds content_mask;
    Corners corner_radii;
    uint2 texture_id;  // u64 in Rust, represented as two uints for alignment
};
...
ExternalTextureVertexOutput external_texture_vertex(uint vertex_id: SV_VertexID, uint texture_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    ExternalTexture ext_tex = external_textures[texture_id];
    float4 device_position = to_device_position(unit_vertex, ext_tex.bounds);
    float4 clip_distance = distance_from_clip_rect(unit_vertex, ext_tex.bounds,
                                                    ext_tex.content_mask);
...
float4 external_texture_fragment(ExternalTextureFragmentInput input): SV_Target {
    ExternalTexture ext_tex = external_textures[input.texture_id];
    float4 sample = t_sprite.Sample(s_sprite, input.texture_position);
    float distance = quad_sdf(input.position.xy, ext_tex.bounds, ext_tex.corner_radii);

    float4 color = sample;
    if ((ext_tex.grayscale & 0xFFu) != 0u) {
        float3 grayscale = dot(color.rgb, GRAYSCALE_FACTORS);
        color = float4(grayscale, sample.a);
    }
    color.a *= ext_tex.opacity * saturate(0.5 - distance);
    return color;
}
