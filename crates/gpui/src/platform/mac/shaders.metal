...
vertex ExternalTextureVertexOutput external_texture_vertex(
    uint unit_vertex_id [[vertex_id]], uint texture_id [[instance_id]],
    constant float2 *unit_vertices [[buffer(SpriteInputIndex_Vertices)]],
    constant ExternalTexture *textures [[buffer(SpriteInputIndex_Sprites)]],
    constant Size_DevicePixels *viewport_size
    [[buffer(SpriteInputIndex_ViewportSize)]],
    constant Size_DevicePixels *texture_size
    [[buffer(SpriteInputIndex_AtlasTextureSize)]]) {
  ...
  float2 unit_vertex = unit_vertices[unit_vertex_id];
  ExternalTexture ext_tex = textures[texture_id];
  float4 device_position =
      to_device_position(unit_vertex, ext_tex.bounds, viewport_size);
  float4 clip_distance = distance_from_clip_rect(unit_vertex, ext_tex.bounds,
                                                 ext_tex.content_mask.bounds);
...
fragment float4 external_texture_fragment(
    ...
    constant ExternalTexture *textures [[buffer(SpriteInputIndex_Sprites)]],
    texture2d<float> ext_texture [[texture(SpriteInputIndex_AtlasTexture)]]) {
  ExternalTexture ext_tex = textures[input.texture_id];
  constexpr sampler texture_sampler(mag_filter::linear,
                                    min_filter::linear);
  float4 sample =
      ext_texture.sample(texture_sampler, input.texture_position);
  float distance =
      quad_sdf(input.position.xy, ext_tex.bounds, ext_tex.corner_radii);

  float4 color = sample;
  if (ext_tex.grayscale) {
    float grayscale = 0.2126 * color.r + 0.7152 * color.g + 0.0722 * color.b;
    color.r = grayscale;
    color.g = grayscale;
    color.b = grayscale;
