...
impl Context {
    /// Get the raw Vulkan device handle
    ///
    /// # Safety
    /// The returned device handle must not outlive the Context
    pub unsafe fn vk_device(&self) -> &ash::Device {
        &self.device.core
    }
    ...
    ///
    /// # Safety
    /// The returned instance handle must not outlive the Context
    pub unsafe fn vk_instance(&self) -> &ash::Instance {
        &self.instance.core
    }

    /// Get the external memory FD extension (Linux only)
    #[cfg(not(target_os = "windows"))]
    pub unsafe fn vk_external_memory_fd(&self) -> Option<&ash::khr::external_memory_fd::Device> {
        self.device.external_memory.as_ref()
    }

    /// Get the VkDeviceMemory for a texture
    ///
    /// # Safety
    /// The texture must have been created by this Context
    pub unsafe fn vk_texture_memory(&self, texture: Texture) -> Option<vk::DeviceMemory> {
        if texture.memory_handle == !0 {
            return None;
        }
        let memory = self.memory.lock().unwrap();
        let block = memory.slab.get(texture.memory_handle)?;
        Some(*block.memory())
    }
}
...
impl Texture {
    ...
    ///
    /// # Safety
    /// The returned image handle must not outlive the Texture
    pub unsafe fn vk_image(&self) -> vk::Image {
        self.raw
    }
}
